...
export function DashboardSidebar({ items, role, roleIcon: RoleIcon, userName, onLogout }: DashboardSidebarProps) {
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-20 bg-gradient-to-b from-[#56CCF2] to-[#3BB5E0] border-r-4 border-[#E07A47] flex flex-col overflow-y-auto z-50 shadow-xl">
      {/* Header - Role Icon */}
      <div className="p-3 border-b border-white/20 flex flex-col items-center">
        <Link href="/dashboard" className="group relative">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#E07A47] to-[#D96835] flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
        ...
        {items.map((item) => {
          ...
          return (
            ...
            >
              <div
                className={cn(
                  "w-14 h-14 mx-auto rounded-2xl flex items-center justify-center transition-all duration-200 relative",
                  isActive
              ...
              >
                ...
                {/* Badge */}
                {item.badge && (
                  <span className="absolute -top-1 -right-1 min-w-[20px] h-5 px-1.5 text-[10px] font-bold rounded-full bg-[#E07A47] text-white flex items-center justify-center shadow-md border-2 border-white">
                    {item.badge}
                  </span>
                )}
              </div>
              ...
              <div
                className={cn(
                  "absolute left-full ml-3 top-1/2 -translate-y-1/2 px-3 py-2 bg-slate-900 text-white text-sm font-bold rounded-lg whitespace-nowrap z-50 pointer-events-none shadow-xl transition-all duration-200",
