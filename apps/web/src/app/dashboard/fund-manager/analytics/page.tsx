      ...
      <main className="flex-1 ml-24 bg-white">
        ...
        <div className="container max-w-7xl px-6 py-8 mx-auto">
          ...
          <Card className="border-4 border-[#E07A47] bg-white mb-6">
            <CardHeader>
              <CardTitle className="text-2xl">Fund Performance Metrics</CardTitle>
            </CardHeader>
            <CardContent>
              ...
              <div className="grid md:grid-cols-3 gap-4 mt-4">
                <div className="bg-muted/50 rounded-lg p-4">
                  <p className="text-sm text-muted-foreground mb-2">Avg Cash-on-Cash</p>
                  <p className="text-2xl font-black text-[#56CCF2]">{performanceMetrics.avgCashOnCash}%</p>
                </div>
                <div className="bg-muted/50 rounded-lg p-4">
                  <p className="text-sm text-muted-foreground mb-2">Avg Cap Rate</p>
                  <p className="text-2xl font-black text-[#E07A47]">{performanceMetrics.avgCapRate}%</p>
                </div>
                <div className="bg-muted/50 rounded-lg p-4">
                  <p className="text-sm text-muted-foreground mb-2">Pref Return Coverage</p>
          ...
          <Card className="border-4 border-[#E07A47] bg-white mb-6">
            <CardHeader>
              <CardTitle className="text-2xl">Asset Performance - YTD</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-slate-200">
                      <th className="text-left py-3 px-4 font-bold">Property</th>
                      <th className="text-center py-3 px-4 font-bold">Units</th>
                      <th className="text-right py-3 px-4 font-bold">Occupancy</th>
                      <th className="text-right py-3 px-4 font-bold">YTD NOI</th>
                      <th className="text-right py-3 px-4 font-bold">Budget</th>
                      <th className="text-right py-3 px-4 font-bold">Variance</th>
                  ...
                  <tbody>
                    {assetPerformance.map((asset) => (
                      <tr key={asset.name} className="border-b border-slate-100">
                        <td className="py-3 px-4 font-bold">{asset.name}</td>
                        <td className="text-center py-3 px-4">{asset.units}</td>
                        ...
                        <td className="text-right py-3 px-4">${(asset.budget / 1000).toFixed(0)}K</td>
          ...
          <Card className="border-4 border-[#56CCF2] bg-white mb-6">
            <CardHeader>
              <CardTitle className="text-2xl">Distribution History</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-slate-200">
                      ...
                      <th className="text-right py-3 px-4 font-bold">Total Amount</th>
                      <th className="text-right py-3 px-4 font-bold">Investors</th>
                      <th className="text-right py-3 px-4 font-bold">Avg/Investor</th>
                      <th className="text-right py-3 px-4 font-bold">Pref Return</th>
                      <th className="text-right py-3 px-4 font-bold">Profit Split</th>
                    </tr>
                  </thead>
                  <tbody>
                    {distributionHistory.map((dist) => (
                      <tr key={dist.quarter} className="border-b border-slate-100">
                        <td className="py-3 px-4 font-bold">{dist.quarter}</td>
                        ...
                        <td className="text-right py-3 px-4">${dist.avgDist.toLocaleString()}</td>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            ...
            <Card className="border-4 border-[#E07A47] bg-white">
              <CardHeader>
                <CardTitle className="text-2xl">Revenue by Asset Type</CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  {revenueByAssetType.map((revenue) => (
                    <div key={revenue.type}>
                      <div className="flex justify-between mb-2">
                        <div>
                          <span className="font-bold">{revenue.type}</span>
            ...
            <Card className="border-4 border-[#E07A47] bg-white">
              <CardHeader>
                <CardTitle className="text-2xl">Investor Concentration</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {investorConcentration.map((bracket) => (
                    <div key={bracket.bracket} className="bg-muted/50 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        <div>
                          <p className="font-bold">{bracket.bracket}</p>
          ...
          <Card className="border-4 border-[#56CCF2] bg-white">
            <CardHeader>
              <CardTitle className="text-2xl">Active Capital Projects</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {capitalProjects.map((project) => (
                  <div key={`${project.property}-${project.project}`} className="bg-muted/50 rounded-lg p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <p className="font-bold">{project.property}</p>
