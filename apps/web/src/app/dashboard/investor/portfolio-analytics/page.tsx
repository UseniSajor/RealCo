      ...
      <main className="flex-1 ml-24 bg-white">
        ...
        <div className="container max-w-7xl px-6 py-8 mx-auto">
          ...
          <Card className="border-4 border-[#E07A47] bg-white mb-6">
            <CardHeader>
              <CardTitle className="text-2xl">Performance by Year</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-slate-200">
                      ...
                      <th className="text-right py-3 px-4 font-bold">Invested</th>
                      <th className="text-right py-3 px-4 font-bold">Value</th>
                      <th className="text-right py-3 px-4 font-bold">Return</th>
                      <th className="text-right py-3 px-4 font-bold">Distributions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {performanceByYear.map((year) => (
                      <tr key={year.year} className="border-b border-slate-100">
                        <td className="py-3 px-4 font-bold">{year.year}</td>
                        <td className="text-right py-3 px-4">${(year.invested / 1000).toFixed(0)}K</td>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            <Card className="border-4 border-[#E07A47] bg-white">
              <CardHeader>
                <CardTitle className="text-2xl">Allocation by Asset Type</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="border-4 border-[#E07A47] bg-white">
              <CardHeader>
                <CardTitle className="text-2xl">Allocation by Location</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {investmentsByLocation.map((location) => (
                    <div key={location.location} className="bg-muted/50 rounded-lg p-4">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-bold">${(location.invested / 1000).toFixed(0)}K</span>
                      </div>
                      <div className="flex justify-between text-sm">
          ...
          <Card className="border-4 border-[#E07A47] bg-white mb-6">
            <CardHeader>
              <CardTitle className="text-2xl">Top Performing Investments</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid md:grid-cols-3 gap-4">
                {topPerformers.map((performer, index) => (
                  <div key={performer.name} className="bg-muted/50 rounded-lg p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Invested:</span>
                        <span className="font-bold">${(performer.invested / 1000).toFixed(0)}K</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Value:</span>
          ...
          <Card className="border-4 border-[#56CCF2] bg-white">
            <CardHeader>
              <CardTitle className="text-2xl">Projected Cash Flow (2024)</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b-2 border-slate-200">
                      ...
                      <th className="text-right py-3 px-4 font-bold">Distributions</th>
                      <th className="text-right py-3 px-4 font-bold">Capital Calls</th>
                      <th className="text-right py-3 px-4 font-bold">Net Cash Flow</th>
                    </tr>
                  </thead>
                  <tbody>
                    {cashFlowProjection.map((period) => (
                      <tr key={period.quarter} className="border-b border-slate-100">
                        <td className="py-3 px-4 font-bold">{period.quarter}</td>
