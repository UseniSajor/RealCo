...
export default function SponsorAnalyticsPage() {
  ...
  const dealVelocity = [
    { month: "Sep", deals: 1, avgDays: 92 },
    { month: "Oct", deals: 2, avgDays: 87 },
    { month: "Nov", deals: 1, avgDays: 94 },
    { month: "Dec", deals: 2, avgDays: 78 },
    { month: "Jan", deals: 1, avgDays: 85 },
  ]
    ...
    <div className="min-h-screen bg-muted/30">
      ...
      <div className="container max-w-7xl px-6 py-8 mx-auto">
        ...
        <Card className="border-4 border-[#E07A47] dark:bg-[#6b7280] mb-6">
          <CardHeader>
            <CardTitle className="text-2xl dark:text-white">Fundraising Performance - {timeframe.toUpperCase()}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid md:grid-cols-4 gap-4 mb-6">
              <div className="bg-green-50 dark:bg-green-900/20 border-2 border-green-500 rounded-lg p-4">
                <p className="text-xs text-muted-foreground dark:text-white/70 mb-1">Capital Raised</p>
                <p className="text-3xl font-black text-green-600">${(fundraisingMetrics.ytdCapitalRaised / 1000000).toFixed(1)}M</p>
                <p className="text-xs text-muted-foreground dark:text-white/70 mt-1">Target: ${(fundraisingMetrics.ytdTarget / 1000000).toFixed(0)}M</p>
              </div>
              <div className="bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-500 rounded-lg p-4">
                <p className="text-xs text-muted-foreground dark:text-white/70 mb-1">Progress to Goal</p>
              ...
              <div className="bg-orange-50 dark:bg-orange-900/20 border-2 border-[#E07A47] rounded-lg p-4">
                <p className="text-xs text-muted-foreground dark:text-white/70 mb-1">Avg Days to Close</p>
              ...
              <div className="bg-purple-50 dark:bg-purple-900/20 border-2 border-purple-500 rounded-lg p-4">
                <p className="text-xs text-muted-foreground dark:text-white/70 mb-1">Conversion Rate</p>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div className="bg-muted/50 dark:bg-slate-700 rounded-lg p-4">
                <p className="text-sm text-muted-foreground dark:text-white/70 mb-2">Avg Investment Size</p>
                <p className="text-2xl font-black text-[#56CCF2]">${(fundraisingMetrics.avgInvestmentSize / 1000).toFixed(0)}K</p>
              </div>
              <div className="bg-muted/50 dark:bg-slate-700 rounded-lg p-4">
                <p className="text-sm text-muted-foreground dark:text-white/70 mb-2">Repeat Investor Rate</p>
                <p className="text-2xl font-black text-green-600">{fundraisingMetrics.repeatInvestorRate}%</p>
              </div>
              <div className="bg-muted/50 dark:bg-slate-700 rounded-lg p-4">
                <p className="text-sm text-muted-foreground dark:text-white/70 mb-2">Investor Growth (YoY)</p>
        ...
        <Card className="border-4 border-[#E07A47] dark:bg-[#6b7280] mb-6">
          <CardHeader>
            <CardTitle className="text-2xl dark:text-white">Performance by Asset Type</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b-2 border-slate-200 dark:border-slate-600">
                    <th className="text-left py-3 px-4 font-bold dark:text-white">Asset Type</th>
                    <th className="text-center py-3 px-4 font-bold dark:text-white">Projects</th>
                    <th className="text-right py-3 px-4 font-bold dark:text-white">Capital Raised</th>
                    <th className="text-right py-3 px-4 font-bold dark:text-white">Avg IRR</th>
                    <th className="text-right py-3 px-4 font-bold dark:text-white">Avg Multiple</th>
                  </tr>
                </thead>
                <tbody>
                  {performanceByAssetType.map((asset) => (
                    <tr key={asset.type} className="border-b border-slate-100 dark:border-slate-700">
                      <td className="py-3 px-4 font-bold dark:text-white">{asset.type}</td>
                      <td className="text-center py-3 px-4 dark:text-white">{asset.projects}</td>
        ...
        <Card className="border-4 border-[#E07A47] dark:bg-[#6b7280] mb-6">
          <CardHeader>
            <CardTitle className="text-2xl dark:text-white">Performance by Market</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {performanceByMarket.map((market) => (
                <div key={market.market} className="bg-muted/50 dark:bg-slate-700 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      <p className="font-bold dark:text-white">{market.market}</p>
                      <p className="text-xs text-muted-foreground dark:text-white/70">{market.projects} projects</p>
                    </div>
                    <Badge className="bg-green-600 text-white">{market.avgIRR}% IRR</Badge>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground dark:text-white/70">Capital Raised:</span>
        ...
        <Card className="border-4 border-[#56CCF2] dark:bg-[#6b7280] mb-6">
          <CardHeader>
            <CardTitle className="text-2xl dark:text-white">Quarterly Fundraising Trend</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b-2 border-slate-200 dark:border-slate-600">
                    ...
                    <th className="text-right py-3 px-4 font-bold dark:text-white">Capital Raised</th>
                    <th className="text-right py-3 px-4 font-bold dark:text-white">New Investors</th>
                    <th className="text-right py-3 px-4 font-bold dark:text-white">Avg Investment</th>
                  </tr>
                </thead>
                <tbody>
                  {quarterlyFundraising.map((quarter) => (
                    <tr key={quarter.quarter} className="border-b border-slate-100 dark:border-slate-700">
                      <td className="py-3 px-4 font-bold dark:text-white">{quarter.quarter}</td>
        ...
        <Card className="border-4 border-[#E07A47] dark:bg-[#6b7280] mb-6">
          <CardHeader>
            <CardTitle className="text-2xl dark:text-white">Investor Segmentation</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {investorSegmentation.map((segment) => (
                <div key={segment.segment}>
                  <div className="flex justify-between mb-2">
                    <div>
                      <span className="font-bold dark:text-white">{segment.segment}</span>
        ...
        <Card className="border-4 border-[#56CCF2] dark:bg-[#6b7280]">
          <CardHeader>
            <CardTitle className="text-2xl dark:text-white">Top 5 Investors</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {topInvestors.map((investor, index) => (
                <div key={investor.name} className="bg-muted/50 dark:bg-slate-700 rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <Badge className="bg-[#56CCF2] text-white">#{index + 1}</Badge>
                      <div>
                        <p className="font-bold dark:text-white">{investor.name}</p>
                        <p className="text-xs text-muted-foreground dark:text-white/70">{investor.deals} deals</p>
                      </div>
                    </div>
                    <div className="text-right">
